(() => {
'use strict';
const KEY='lf_travel_expense_v1';
const $=id=>document.getElementById(id);
const today=()=>new Date().toISOString().slice(0,10);
const load=()=>{try{return JSON.parse(localStorage.getItem(KEY))||{work:[],expenses:[]}}catch{return{work:[],expenses:[]}}};
const save=d=>localStorage.setItem(KEY,JSON.stringify(d));
const isWeekend=s=>{const d=new Date(s+'T12:00:00');return d.getDay()===0||d.getDay()===6};
function hhmmMinutes(v){const s=String(v||'').replace(/\D/g,'').padStart(4,'0');if(!/^\d{4}$/.test(s))return null;const h=+s.slice(0,2),m=+s.slice(2);if(h>23||m>59)return null;return h*60+m}
function calcOT(date,start,end,brk){const s=hhmmMinutes(start),e=hhmmMinutes(end),b=Number(brk)||0;if(s===null||e===null||e<s)return null;if(isWeekend(date)){const mins=Math.max(0,e-s-b);return mins>=30?mins:0}const after=Math.max(0,e-(18*60+30));return after>=30?after:0}
function fmtMins(m){m=Number(m)||0;return Math.floor(m/60)+'h '+(m%60)+'m'}
function hkdLimit(type){return type==='早餐'?100:type==='午餐'?150:200}
function setDefaults(){['workDate','expenseDate'].forEach(id=>$(id).value=today());syncCity()}
...
setDefaults();previewWork();render();
})();